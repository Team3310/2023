...
public class PutIntakeZeroAfterOuttake extends SequentialCommandGroup {
    ...
    public PutIntakeZeroAfterOuttake(Intake intake, Arm arm) {
        ...
        this.addCommands(new InstantCommand(()->this.intake.setArmIntakeSpeed(0)));
        // if(!intake.getConeSensor().get() && !this.intake.getCubeSensor().get()){ 
            //removed logic because it was causing some issues where it wouldn't go down
