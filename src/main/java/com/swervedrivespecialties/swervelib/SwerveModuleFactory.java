...
public class SwerveModuleFactory<DriveConfiguration, SteerConfiguration> {
    ...
    private static class ModuleImplementation implements SwerveModule {
        ...
        @Override
        public void setDriveNeutralMode(NeutralMode neutralMode){driveController.setMotorNeutralMode(neutralMode);}
        
        @Override
        public void setSteerNeutralMode(NeutralMode neutralMode){steerController.setMotorNeutralMode(neutralMode);}
        ...
        @Override
        public double getTargetDriveVoltage() {
            return driveController.getReferenceVoltage();
        }
        ...
        @Override
        public void setVelocity(double metersPerSec, double steerAngle) {
            steerAngle %= (2.0 * Math.PI);
            if (steerAngle < 0.0) {
                steerAngle += 2.0 * Math.PI;
            }
            ...
            driveController.setDriveVelocity(metersPerSec);
            steerController.setReferenceAngle(steerAngle);
        }
    }
}
